# fetch_meta_performance_daily.py
...
DATA_PATH = os.path.join(os.path.dirname(__file__), '..', 'Data', 'API_Data', 'Meta', 'meta_performance_daily.parquet')
...
def fetch_and_save_meta_marketing_data():
    ...
    for account_config in META_ACCOUNTS:
        ...
        for date_str in missing_dates:
            print(f"Veri çekiliyor: Account {current_account_id}, Tarih {date_str}")
            ...
            base_params = {
                ...
                'fields': [
                    ...
                    'account_id', 'account_name', 'impressions', 'reach', 'clicks', 'unique_clicks',
                    'spend'
                ],
            ...
            try:
                ads = account.get_insights(
                    fields=base_params['fields'],
                    params={k: v for k, v in base_params.items() if k != 'fields'}
                )
            except Exception as e:
                print(f"Hata oluştu: {e} — Tarih: {date_str}, Account: {current_account_id}")
                continue
            ...
            for ad in ads:
                try:
                    ad_data = {
                        ...
                        "unique_clicks": ad.get("unique_clicks", 0),
    ...
    if all_data:
        ...
        # Overwrite same-date rows before append
        ...
        print(f"Meta Marketing verisi güncellendi: {DATA_PATH}")
    else:
        print("Hiçbir yeni veri bulunamadı. Dosya güncellenmedi.")
